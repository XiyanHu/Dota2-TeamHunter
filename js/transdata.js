$(document).ready(function() {
				...
				httpRequest("https://api.opendota.com/api/proPlayers/", function(result){
				    ...
				    var count = 0;
				    for (var i=1,teamListLength=team_list.length;i<teamListLength;i++){
				    		var k;
				    		for (k=0;k<teamArr.length;k++){
				    			if (teamArr[k]==team_list[i].team_id){
				    				k --;break;}}
				    		if (k==teamArr.length){
				    			count ++;teamArr.push(team_list[i].team_id);
				...
				$('#selectTeam').editableSelect().on('select.editable-select', function (e, li) {				
					...
					if (localStorage["testfield"]){ $("#loadingTeamInfo").css('display','block');	}				
