//If there is value in localstorage, draw the match table
...
async function showMe(team_id, team_name) {
    ...
    try {
        const team_info = await fetchRequest("https://api.opendota.com/api/proPlayers/");
        ...
        const r = await fetchRequest(recentGameUrl);
        const len1 = r.rowCount;
        if (len1 == 0) {
        ...
        if (localStorage["firstMatchId"] != r.rows[0].match_id) {
            chrome.action.setIcon({ path: { '16': 'images/icon16_notify.png' } });
        } else {
            chrome.action.setIcon({ path: { '16': 'images/icon16.png' } });
        }
        ...
        for (var i = 0; i < len1; i++) {
            ...
            const detailResponse = await fetch(`https://api.opendota.com/api/matches/${r.rows[i].match_id}`);
